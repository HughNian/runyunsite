...
require_once(DEDEINC."/phpqrcode/qrlib.php");
...
else if($dopost=='save')
{
    ...
    if($type == 1){
    	$querystring = "SELECT COUNT(*) AS count FROM `#@__archives` WHERE `typeid` = {$typeid} AND `type` = 1";
    	$count = $dsql->GetOne($querystring);
    	if($count['count'] == 1) {
    		ShowMsg("每个栏目只能有一篇更多模板文章!","-1");
    		exit();
    	}
    }
    ...
    //生成文档ID
    $arcID = GetIndexKey($arcrank,$typeid,$sortrank,$channelid,$senddate,$adminid,$type);
    if(empty($arcID))
    {
    ...
    if($link){
    	...
    	$qrcode_dir = '/uploads/qrcode/'. strtr(date('Y-m-d-H', time()), '-','/') .'/';
    	$direct_dir = $base_dir . $qrcode_dir;
    	if(!is_dir($direct_dir)){
    		mkdir($direct_dir, 0777, true);
    	}
    	$direct = $direct_dir . $filename;
    	QRcode::png($link, $direct, 6, 15);
    	if(file_exists($direct)){
    		$qrcode = $qrcode_dir . $filename;
    	} else {
    		$qrcode = "";
    	}	
    }
    ...
    $query = "INSERT INTO `#@__archives`(id,typeid,typeid2,sortrank,flag,ismake,channel,arcrank,click,money,title,shorttitle,
     color,writer,source,litpic,pubdate,senddate,mid,notpost,description,keywords,filename,dutyadmin,weight,tag,type,link,qrcode)
    VALUES ('$arcID','$typeid','$typeid2','$sortrank','$flag','$ismake','$channelid','$arcrank','$click','$money','$title','$shorttitle',
    '$color','$writer','$source','$litpic','$pubdate','$senddate','$adminid','$notpost','$description','$keywords','$filename','$adminid','$weight','$tags','$type','$link','$qrcode'); ";
